...
START_TEST(success_sub) {
  ...
  const double b[4][3] = {{456.234523, 5E-8, -5E-8},
                          ...
                          {4E-8, 310, 1.0 / 3.0}};
  const double r[4][3] = {{-912.469046, 0, 1E-7},
                          {53456345634563.9, 0, -106913066.0806907},
                          {-9E-7, 0, 19E14},
                          {0, -187, 0.3333333}};
  matrix_t A, B, R;
...
START_TEST(null_args) {
  ...
  ck_assert_int_eq(s21_sub_matrix(&B, A, &R), 1);
  ck_assert_int_eq(s21_sub_matrix(&B, &B, A), 1);
  s21_remove_matrix(&B);
}
END_TEST
...
START_TEST(invalid_row) {
  ...
  ck_assert_int_eq(s21_sub_matrix(&A, &B, &R), 1);
  ck_assert_int_eq(s21_sub_matrix(&B, &A, &R), 1);
  ck_assert_int_eq(s21_sub_matrix(&A, &A, &R), 1);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
}
END_TEST
